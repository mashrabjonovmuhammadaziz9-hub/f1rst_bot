...
BOT_TOKEN = os.getenv("BOT_TOKEN", "YANGI_TOKENNI_SHU_YERGA_QOY")
...
def create_database():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS serial_episodes (
            ...
            file_type TEXT NOT NULL,
            created_at TEXT,
            UNIQUE(serial_id, season, episode),
...
def admin_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("📺 Serial qo‘shish", callback_data="admin_add_serial"),
            InlineKeyboardButton("➕ Qism qo‘shish", callback_data="admin_add_episode"),
        ],
        [
            InlineKeyboardButton("📺 Seriallar", callback_data="admin_serials"),
            InlineKeyboardButton("🗑 Serial o‘chirish", callback_data="admin_delete_serial"),
        ],
        [
...
async def admin_buttons(update, context):
    ...
    elif data == "admin_serials":
        rows = get_serials()
        if not rows:
            await q.message.reply_text("📺 Hali serial yo‘q.")
        else:
            text = "📺 SERIALLAR\n\n"
            for code, title, premium in rows:
                text += f"{'💎' if premium else '🆓'} {code} — {title}\n"
            await q.message.reply_text(text[:4000])

    elif data == "admin_delete_serial":
        await q.message.reply_text("🗑 Serialni o‘chirish:\n/delserial KOD\n\nMasalan: /delserial 100")
...
async def menu_message(update, context):
    ...
    if text.isdigit():
        if await show_serial(update, context, text):
            return
...
SER_TITLE, SER_CODE, SER_SEASON, SER_EPISODE, SER_VIDEO, SER_TYPE = range(20, 26)
EP_CODE, EP_SEASON, EP_EPISODE, EP_VIDEO = range(30, 34)


def get_serial(code):
    conn = db(); c = conn.cursor()
    c.execute("SELECT id, code, title, premium FROM serials WHERE code=?", (str(code),))
    row = c.fetchone(); conn.close(); return row


def get_serials():
    conn = db(); c = conn.cursor()
    c.execute("SELECT code, title, premium FROM serials ORDER BY id DESC LIMIT 50")
    rows = c.fetchall(); conn.close(); return rows


def get_serial_seasons(serial_id):
    conn = db(); c = conn.cursor()
    c.execute("SELECT DISTINCT season FROM serial_episodes WHERE serial_id=? ORDER BY season", (serial_id,))
    rows = [r[0] for r in c.fetchall()]; conn.close(); return rows


def get_serial_episodes(serial_id, season):
    conn = db(); c = conn.cursor()
    c.execute("SELECT episode FROM serial_episodes WHERE serial_id=? AND season=? ORDER BY episode", (serial_id, season))
    rows = [r[0] for r in c.fetchall()]; conn.close(); return rows


def save_serial(title, code, season, episode, file_id, file_type, premium):
    conn = db(); c = conn.cursor()
    try:
        c.execute("INSERT INTO serials(code,title,premium,created_at) VALUES(?,?,?,?)",
                  (code,title,premium,datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        sid = c.lastrowid
        c.execute("INSERT INTO serial_episodes(serial_id,season,episode,file_id,file_type,created_at) VALUES(?,?,?,?,?,?)",
                  (sid,season,episode,file_id,file_type,datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
def save_episode(code, season, episode, file_id, file_type):
    serial = get_serial(code)
    if not serial: return False, "Serial topilmadi"
    conn=db(); c=conn.cursor()
    try:
        c.execute("INSERT INTO serial_episodes(serial_id,season,episode,file_id,file_type,created_at) VALUES(?,?,?,?,?,?)",
                  (serial[0],season,episode,file_id,file_type,datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit(); return True, serial[2]
    except sqlite3.IntegrityError:
        return False, "Bu mavsum/qism avval qo‘shilgan"
    finally:
        conn.close()


async def add_serial_start(update, context):
    q=update.callback_query; await q.answer()
    if q.from_user.id != ADMIN_ID: return ConversationHandler.END
    context.user_data['new_serial']={}
    await q.message.reply_text("📺 Yangi serial qo‘shish\n\n1️⃣ Serial nomini yozing:\n/cancel — bekor qilish")
    return SER_TITLE


async def ser_title(update, context):
    context.user_data['new_serial']['title']=update.message.text.strip()
    await update.message.reply_text("2️⃣ Serial kodini yozing (masalan 100):")
    return SER_CODE


async def ser_code(update, context):
    code=update.message.text.strip()
    if get_serial(code) or get_movie(code):
        await update.message.reply_text("❌ Bu kod band. Boshqa kod yozing:"); return SER_CODE
    context.user_data['new_serial']['code']=code
    await update.message.reply_text("3️⃣ Mavsum raqamini yozing (masalan 1):")
    return SER_SEASON


async def ser_season(update, context):
    try: n=int(update.message.text)
    except: await update.message.reply_text("❌ Faqat raqam yozing:"); return SER_SEASON
    context.user_data['new_serial']['season']=n
    await update.message.reply_text("4️⃣ Qism raqamini yozing (masalan 1):")
    return SER_EPISODE


async def ser_episode(update, context):
    try: n=int(update.message.text)
    except: await update.message.reply_text("❌ Faqat raqam yozing:"); return SER_EPISODE
    context.user_data['new_serial']['episode']=n
    await update.message.reply_text("5️⃣ Shu qism videosini/faylini yuboring:")
    return SER_VIDEO


async def ser_video(update, context):
    m=update.message
    if m.video: fid,ft=m.video.file_id,'video'
    elif m.document: fid,ft=m.document.file_id,'document'
    elif m.animation: fid,ft=m.animation.file_id,'animation'
    else: await m.reply_text("❌ Video yoki fayl yuboring:"); return SER_VIDEO
    context.user_data['new_serial'].update(file_id=fid,file_type=ft)
    await m.reply_text("6️⃣ Serial turini tanlang:", reply_markup=InlineKeyboardMarkup([[
        InlineKeyboardButton("🆓 Oddiy",callback_data="serial_normal"),
        InlineKeyboardButton("💎 Premium",callback_data="serial_premium")]]))
    return SER_TYPE


async def ser_type(update, context):
    q=update.callback_query; await q.answer(); d=context.user_data.get('new_serial',{})
    premium=1 if q.data=='serial_premium' else 0
    ok=save_serial(d['title'],d['code'],d['season'],d['episode'],d['file_id'],d['file_type'],premium)
    await q.message.reply_text((f"✅ SERIAL SAQLANDI!\n📺 {d['title']}\n🔢 Kod: {d['code']}\n📚 {d['season']}-mavsum, {d['episode']}-qism") if ok else "❌ Serial saqlanmadi.")
    context.user_data.pop('new_serial',None); return ConversationHandler.END


async def add_episode_start(update, context):
    q=update.callback_query; await q.answer()
    if q.from_user.id != ADMIN_ID: return ConversationHandler.END
    context.user_data['new_episode']={}
    await q.message.reply_text("➕ Serialga qism qo‘shish\n\n1️⃣ Serial kodini yozing:\n/cancel — bekor qilish")
    return EP_CODE


async def ep_code(update, context):
    code=update.message.text.strip()
    if not get_serial(code): await update.message.reply_text("❌ Serial topilmadi. Kodni qayta yozing:"); return EP_CODE
    context.user_data['new_episode']['code']=code
    await update.message.reply_text("2️⃣ Mavsum raqami:"); return EP_SEASON


async def ep_season(update, context):
    try: n=int(update.message.text)
    except: await update.message.reply_text("❌ Faqat raqam:"); return EP_SEASON
    context.user_data['new_episode']['season']=n
    await update.message.reply_text("3️⃣ Qism raqami:"); return EP_EPISODE


async def ep_episode(update, context):
    try: n=int(update.message.text)
    except: await update.message.reply_text("❌ Faqat raqam:"); return EP_EPISODE
    context.user_data['new_episode']['episode']=n
    await update.message.reply_text("4️⃣ Qism videosini/faylini yuboring:"); return EP_VIDEO


async def ep_video(update, context):
    m=update.message
    if m.video: fid,ft=m.video.file_id,'video'
    elif m.document: fid,ft=m.document.file_id,'document'
    elif m.animation: fid,ft=m.animation.file_id,'animation'
    else: await m.reply_text("❌ Video yoki fayl yuboring:"); return EP_VIDEO
    d=context.user_data['new_episode']; ok,msg=save_episode(d['code'],d['season'],d['episode'],fid,ft)
    await m.reply_text(f"✅ {msg}: {d['season']}-mavsum {d['episode']}-qism qo‘shildi." if ok else f"❌ {msg}")
    context.user_data.pop('new_episode',None); return ConversationHandler.END


async def cancel_serial(update, context):
    context.user_data.pop('new_serial',None); context.user_data.pop('new_episode',None)
    await update.message.reply_text("❌ Bekor qilindi."); return ConversationHandler.END


async def delserial_command(update, context):
    if update.effective_user.id != ADMIN_ID: return
    if not context.args: await update.message.reply_text("Foydalanish: /delserial KOD"); return
    conn=db(); c=conn.cursor(); c.execute("SELECT id FROM serials WHERE code=?",(context.args[0],)); row=c.fetchone()
    if not row: conn.close(); await update.message.reply_text("❌ Serial topilmadi."); return
    c.execute("DELETE FROM serial_episodes WHERE serial_id=?",(row[0],)); c.execute("DELETE FROM serials WHERE id=?",(row[0],)); conn.commit(); conn.close()
    await update.message.reply_text("🗑 Serial va uning barcha qismlari o‘chirildi.")


async def show_serial(update, context, code):
    serial=get_serial(code)
    if not serial: return False
    sid,code,title,premium=serial
    if premium and not is_premium(update.effective_user.id):
        await update.effective_message.reply_text("💎 Bu Premium serial. Premium olish uchun menyudan Premium bo‘limiga kiring.")
        return True
    seasons=get_serial_seasons(sid)
    if not seasons: await update.effective_message.reply_text("📺 Hozircha qismlar qo‘shilmagan."); return True
    kb=[[InlineKeyboardButton(f"{x}-mavsum",callback_data=f"serseason:{sid}:{x}")] for x in seasons]
    await update.effective_message.reply_text(f"📺 {title}\n\nMavsumni tanlang:",reply_markup=InlineKeyboardMarkup(kb)); return True


async def serial_callback(update, context):
    q=update.callback_query; await q.answer(); parts=q.data.split(':')
    if parts[0]=='serseason':
        sid,season=int(parts[1]),int(parts[2]); eps=get_serial_episodes(sid,season)
        kb=[]
        row=[]
        for ep in eps:
            row.append(InlineKeyboardButton(f"{ep}-qism",callback_data=f"serep:{sid}:{season}:{ep}"))
            if len(row)==3: kb.append(row); row=[]
        if row: kb.append(row)
        await q.message.reply_text(f"📚 {season}-mavsum. Qismni tanlang:",reply_markup=InlineKeyboardMarkup(kb))
    elif parts[0]=='serep':
        sid,season,episode=map(int,parts[1:4]); conn=db(); c=conn.cursor()
        c.execute("SELECT s.title,e.file_id,e.file_type FROM serial_episodes e JOIN serials s ON s.id=e.serial_id WHERE e.serial_id=? AND e.season=? AND e.episode=?",(sid,season,episode)); row=c.fetchone(); conn.close()
        if not row: await q.message.reply_text("❌ Qism topilmadi."); return
        title,fid,ft=row; cap=f"📺 {title}\n📚 {season}-mavsum | 🎞 {episode}-qism"
        if ft=='video': await q.message.reply_video(fid,caption=cap)
        elif ft=='animation': await q.message.reply_animation(fid,caption=cap)
        else: await q.message.reply_document(fid,caption=cap)


# Render Web Service uchun kichik HTTP health server
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200); self.send_header("Content-Type","text/plain; charset=utf-8"); self.end_headers(); self.wfile.write(b"F1RST bot is running")
...
def main():
    ...
    add_serial_conversation = ConversationHandler(
        entry_points=[CallbackQueryHandler(add_serial_start, pattern="^admin_add_serial$")],
        states={
            SER_TITLE:[MessageHandler(filters.TEXT & ~filters.COMMAND, ser_title)],
            SER_CODE:[MessageHandler(filters.TEXT & ~filters.COMMAND, ser_code)],
            SER_SEASON:[MessageHandler(filters.TEXT & ~filters.COMMAND, ser_season)],
            SER_EPISODE:[MessageHandler(filters.TEXT & ~filters.COMMAND, ser_episode)],
            SER_VIDEO:[MessageHandler(filters.VIDEO | filters.Document.ALL | filters.ANIMATION, ser_video)],
            SER_TYPE:[CallbackQueryHandler(ser_type, pattern="^serial_(normal|premium)$")],
        }, fallbacks=[CommandHandler("cancel", cancel_serial)]
    )

    add_episode_conversation = ConversationHandler(
        entry_points=[CallbackQueryHandler(add_episode_start, pattern="^admin_add_episode$")],
        states={
            EP_CODE:[MessageHandler(filters.TEXT & ~filters.COMMAND, ep_code)],
            EP_SEASON:[MessageHandler(filters.TEXT & ~filters.COMMAND, ep_season)],
            EP_EPISODE:[MessageHandler(filters.TEXT & ~filters.COMMAND, ep_episode)],
            EP_VIDEO:[MessageHandler(filters.VIDEO | filters.Document.ALL | filters.ANIMATION, ep_video)],
        }, fallbacks=[CommandHandler("cancel", cancel_serial)]
    )
    ...
    application.add_handler(CommandHandler("delserial", delserial_command))
    ...
    application.add_handler(add_serial_conversation)
    application.add_handler(add_episode_conversation)
    ...
    application.add_handler(CallbackQueryHandler(serial_callback, pattern="^(serseason|serep):"))
    application.add_handler(
        CallbackQueryHandler(
            admin_buttons,
            pattern="^admin_"
